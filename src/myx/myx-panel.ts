...
const setUpdateDataTransferredInterval = (labelRegistry: Label, labelTraffic: Label, tilesLoadedLabel: Label) => {
    ...
    const getAssetSize = (asset: Asset) => {
        ...
        const res = asset.resource as { splatData?: { elements: Array<{ properties: Array<{ storage: { byteLength: number } }> }> }; elements?: Array<{ properties: Array<{ storage: { byteLength: number } }> }> };
        const splatData = res.splatData ?? res;
    ...
    setInterval(() => {
        // @ts-ignore
        const registry = window.assetLoader.registry;
        ...
        // @ts-ignore
        const loadedTiles = window.loadedTiles;
        // @ts-ignore
        const lod = window.lod;
        if (!lod || !loadedTiles) {
...
class MyxPanel extends Container {
    constructor(events: Events, config: SceneConfig, args = {}) {
        ...
        removeAllButton.dom.addEventListener('click', (ev: MouseEvent) => {
            for (const elem of window.scene.elements) {
                if (elem.type !== 'splat') {
                    continue;
                }
                setTimeout(() => {
